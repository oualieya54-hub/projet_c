...
{
    GtkWidget *window_ajout = lookup_widget(GTK_WIDGET(button), "AjoutSalle");
...
if (!au_moins_un) {
    ...
    SalleSport s;
    s.id_salle = generer_nouvel_id("data/salle.txt");
    strcpy(s.nom, nom);
    ...
    if (ajouter_salle("data/salle.txt", s))
    {
        // Message de succès
        GtkWidget *dialog = gtk_message_dialog_new(
            GTK_WINDOW(window_ajout),
...
void
salle_on_button36_clicked                    (GtkButton       *button,
                                        gpointer         user_data)
{
    GtkWidget *window = lookup_widget(GTK_WIDGET(button), "AjoutSalle");
    gtk_widget_destroy(window);
}


void
salle_on_esp__ce1_activate                   (GtkMenuItem     *menuitem,
                                        gpointer         user_data)
{
...
void
salle_on_ch__que1_activate                   (GtkMenuItem     *menuitem,
                                        gpointer         user_data)
{
...
void
salle_on_cartebancaire1_activate             (GtkMenuItem     *menuitem,
                                        gpointer         user_data)
{
...
void
salle_on_virement1_activate                  (GtkMenuItem     *menuitem,
                                        gpointer         user_data)
{
...
static gboolean rafraichir_treeview_idle(gpointer data)
{
    ...
    afficher_salles_interface(refresh->treeview, "data/salle.txt");
...
{
    GtkWidget *window_gestion = lookup_widget(GTK_WIDGET(button), "GestionSalle");
    GtkWidget *treeview = lookup_widget(window_gestion, "treeview3");
    ...
    if (response == GTK_RESPONSE_YES)
    {
        ...
        int resultat = supprimer_salle("data/salle.txt", id_salle);
...
void salle_on_button33_clicked(GtkButton *button, gpointer user_data)
{
    ...
    salle_open_modify_window(model, &iter);
}
...
void
salle_on_button45_clicked                    (GtkButton       *button,
                                        gpointer         user_data)
{
...
if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lookup_widget(window_modif, "checkbutton8")))) {
    ...
    int resultat = modifier_salle("data/salle.txt", id_salle, s);
...
void salle_on_inscri_salle_clicked(GtkButton *button, gpointer user_data)
{
    ...
    FILE *fs = fopen("data/salle.txt", "r");
    SalleSport s_temp;
    int i = 0;
    ...
    FILE *fe = fopen("data/entraineurs.txt", "r");
...
void salle_remplir_listes_inscription_salle(GtkWidget *window) {
    ...
    // Remplir les menus
    char **items_salles = (char**) g_new(char*, nb_salles_stored);
    for (int i = 0; i < nb_salles_stored; i++) {
    ...
    char **items_entr = (char**) g_new(char*, nb_entraineurs_stored);
    for (int i = 0; i < nb_entraineurs_stored; i++) {
...
salle_on_button49_clicked                    (GtkButton       *button,
                                        gpointer         user_data)
{
  GtkWidget *window = gtk_widget_get_toplevel(GTK_WIDGET(button));
